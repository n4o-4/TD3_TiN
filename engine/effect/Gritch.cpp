...
void Gritch::Initialize(DirectXCommon* dxCommon, SrvManager* srvManager)
{
	// パイプラインの生成
...
void Gritch::Draw(uint32_t renderTargetIndex, uint32_t renderResourceIndex)
{
	// 描画先のRTVのインデックス
...
void Gritch::CreateRootSignature(Pipeline* pipeline)
{
	HRESULT hr;
...
void Gritch::CreatePipeLineState(Pipeline* pipeline)
{
	...
	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = dxCommon_->CompileShader(L"Resources/shaders/Gritch.PS.hlsl", L"ps_6_0");
...
void Gritch::CreateMaterial()
{
	// bufferResourceの生成
	resource_ = DirectXCommon::GetInstance()->CreateBufferResource(sizeof(GritchShader::Material));
