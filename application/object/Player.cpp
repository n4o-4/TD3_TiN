...
Vector3 Player::GetMovementInput() {
	...
	if (Input::GetInstance()->PushKey(DIK_W)) {
		rawInputDirection.z += 1.0f;
	}
	if (Input::GetInstance()->PushKey(DIK_S)) {
		rawInputDirection.z -= 1.0f;
	}
	if (Input::GetInstance()->PushKey(DIK_A)) {
		rawInputDirection.x -= 1.0f;
	}
...
bool Player::HandleQuickBoostActivation() {
	if (quickBoostCooldown_ <= 0.0f &&
		...
		!isQuickBoosting_) {
		// isQuickBoosting_ のチェックは ProcessActiveQuickBoost で行われるため、ここでは不要かもしれないが念のため

		Vector3 rawBoostDirection = {0.0f, 0.0f, 0.0f};
		if (Input::GetInstance()->PushKey(DIK_W)) {
			rawBoostDirection.z += 1.0f;
		}
		if (Input::GetInstance()->PushKey(DIK_S)) {
			rawBoostDirection.z -= 1.0f;
		}
		if (Input::GetInstance()->PushKey(DIK_A)) {
			rawBoostDirection.x -= 1.0f;
		}
		if (Input::GetInstance()->PushKey(DIK_D)) {
			rawBoostDirection.x += 1.0f;
		}
